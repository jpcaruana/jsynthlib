/* Most of this copied from BankDriver.java, but that seemed better
 * than copying everything from YamahaMotifSingleDriver. Now I see why
 * multiple inheritance can be good, so I'll probably include it in my
 * language now. */
/*package synthdrivers.YamahaMotif;
import core.*;
import javax.swing.*;
public abstract class YamahaMotifBankDriver extends YamahaMotifSingleDriver {
  ...
  public void setPatchName(Patch p, String name) {}
         /**Most Banks have no name
  public String getPatchName(Patch p) {return "-";}
  public void storePatch (Patch p, int bankNum,int patchNum)
    { sendPatchWorker(p, -1, true); }
  public void sendPatch (Patch p)
    {JOptionPane.showMessageDialog(null,
       "You can not send bank data (use store)","Error", JOptionPane.ERROR_MESSAGE);
    }
  ...
  {
    int bank=0;
    if (bankNumbers.length==1)  {storePatch(p,0,0); return;};
    try{
  ...
  public Patch getPatch(Patch bank,int patchNum) {return null;}
         /**Get the name of the patch at the given number
  public String getPatchName(Patch p,int patchNum) {
      return getPatchName(getPatch(p, patchNum));
  }
         /**Banks cannot play
  public void playPatch(Patch p)  
    {JOptionPane.showMessageDialog(null,
       "Can not Play Banks, only individual patches.","Error", JOptionPane.ERROR_MESSAGE);
     
    }
  public void setPatchName(Patch p,int patchNum, String name){
      setPatchName(getPatch(p, patchNum), name);
  }
...
/**Compares the header & size of a Single Patch to this driver to see if this
   bank can hold the patch
