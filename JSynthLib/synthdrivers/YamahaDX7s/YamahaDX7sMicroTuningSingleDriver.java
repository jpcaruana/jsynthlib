 ...
 * file:    YamahaDX7sMicroTuningSingleDriver.java
 * date:    25.02.2003
 * @version 0.1
 *
...
public class YamahaDX7sMicroTuningSingleDriver extends Driver
{
  ...
  public YamahaDX7sMicroTuningSingleDriver()
  {
    ...
    checksumStart=6;
    checksumEnd=271;
    patchNumbers = DX7sConstants.PATCH_NUMBERS_MICRO_TUNING_SINGLE;
    bankNumbers  = DX7sConstants.BANK_NUMBERS_MICRO_TUNING_SINGLE;
    patchSize=274;
    trimSize=274;
    numSysexMsgs=1;         
  ...
  {
    Patch p = new Patch(DX7sConstants.INIT_MICRO_TUNING,getDeviceNum(),getDriverNum());
  ...
  public JInternalFrame editPatch(Patch p)
  {
    if ( ((YamahaDX7sDevice)(PatchEdit.appConfig.getDevice(getDeviceNum()))).getSpinnerEditorFlag()==1 )
    {
      ...
      {
	return new YamahaDX7sMicroTuningEditor2(p);	// use of JSpinner requires jdk>=1.4 !
      }
      else
      {
	...
	return new YamahaDX7sMicroTuningEditor(p);
      }
    }
    else
      return new YamahaDX7sMicroTuningEditor(p);
