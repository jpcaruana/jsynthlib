 ...
 * file:    YamahaTX7PerformanceBankDriver.java
 * date:    25.02.2003
 * @version 0.1
 *
...
public class YamahaTX7PerformanceBankDriver extends BankDriver
{
  ...
  public JInternalFrame editPatch(Patch p)
  {
    if ( ((YamahaTX7Device)(PatchEdit.appConfig.getDevice(getDeviceNum()))).getTipsMsgFlag()==1 )
       JOptionPane.showMessageDialog(PatchEdit.instance,
                                     getDriverName()+"Driver:"+
                                     TX7Strings.EDIT_BANK_PERFORMANCE_STRING,
                                     getDriverName()+"Driver",
                                     JOptionPane.INFORMATION_MESSAGE);
  ...
  {
    int start=(64*patchNum); // single-patch-size in bulked-dump-packed-format
  ...
  public String getPatchName(Patch p,int patchNum)
  {
    ...
    nameStart+=34;                        //offset of name in bulk patch data

    try
    {
      StringBuffer s= new StringBuffer(new String(p.sysex,nameStart,30,"US-ASCII")); // patchname size
      return s.toString();
  ...
  {
    patchNameSize=30;
    patchNameStart=getPatchStart(patchNum)+34;
  ...
  public void putPatch(Patch bank,Patch p,int patchNum) //puts a patch into the bank, converting it as needed
  {
    ...
    // Transform Voice Data to Bulk Dump Packed Format
                                                         	                             //    ***** Voice A *****
    bank.sysex[getPatchStart(patchNum)+ 0]=(byte)(p.sysex[6+ 2]*64+patchNum);                // Poly/Mono ..............(0- 1)| UNDOCUMENTED! matching voice#(0-63)
    bank.sysex[getPatchStart(patchNum)+ 1]=(byte)((p.sysex[6+ 4]&7)*16+p.sysex[6+ 3]);       // Pitch Bend Step (Low)...(0-12)| Pitch Bend Range ............(0-12)
    bank.sysex[getPatchStart(patchNum)+ 2]=(byte)(p.sysex[6+ 5]);                            // Portamento Time ........(0-99)
    bank.sysex[getPatchStart(patchNum)+ 3]=(byte)(p.sysex[6+ 7]* 2+p.sysex[6+ 6]);           // Portamento Mode ........(0- 1)| Portamento/Glissando ........(0- 1)
    bank.sysex[getPatchStart(patchNum)+ 4]=(byte)(p.sysex[6+10]*16+p.sysex[6+ 9]);           // Modulation Wheel Assign (0- 7)| Modulation Wheel Sensitivity (0-15)
    bank.sysex[getPatchStart(patchNum)+ 5]=(byte)(p.sysex[6+12]*16+p.sysex[6+11]);           // Foot Control Assign ....(0- 7)| Foot Control Sensitivity ....(0-15)
    bank.sysex[getPatchStart(patchNum)+ 6]=(byte)(p.sysex[6+14]*16+p.sysex[6+13]);           // Aftertouch Assign ......(0- 7)| Aftertouch Sensitivity ......(0-15)
    bank.sysex[getPatchStart(patchNum)+ 7]=(byte)(p.sysex[6+16]*16+p.sysex[6+15]);           // Breath Control Assign ..(0- 7)| Breath Control Sensitivity ..(0-15)
    ...
    bank.sysex[getPatchStart(patchNum)+15]=(byte)(((p.sysex[6+ 4]&15)>>3)*64);		     // Pitch Bend Step (High)..(0-12)
                                                                                             ...
                                                                                             //    ***** Voice B *****
    bank.sysex[getPatchStart(patchNum)+16]=(byte)(p.sysex[6+32]*64+patchNum);                // Poly/Mono ..............(0- 1)| UNDOCUMENTED! matching voice#(0-63)
    bank.sysex[getPatchStart(patchNum)+17]=(byte)((p.sysex[6+34]&7)*16+p.sysex[6+33]);       // Pitch Bend Step (Low)...(0-12)| Pitch Bend Range ............(0-12)
    bank.sysex[getPatchStart(patchNum)+18]=(byte)(p.sysex[6+35]);                            // Portamento Time ........(0-99)
    bank.sysex[getPatchStart(patchNum)+19]=(byte)(p.sysex[6+37]* 2+p.sysex[6+36]);           // Portamento Mode ........(0- 1)| Portamento/Glissando ........(0- 1)
    bank.sysex[getPatchStart(patchNum)+20]=(byte)(p.sysex[6+40]*16+p.sysex[6+39]);           // Modulation Wheel Assign (0- 7)| Modulation Wheel Sensitivity (0-15)
    bank.sysex[getPatchStart(patchNum)+21]=(byte)(p.sysex[6+42]*16+p.sysex[6+41]);           // Foot Control Assign ....(0- 7)| Foot Control Sensitivity ....(0-15)
    bank.sysex[getPatchStart(patchNum)+22]=(byte)(p.sysex[6+44]*16+p.sysex[6+43]);           // Aftertouch Assign ......(0- 7)| Aftertouch Sensitivity ......(0-15)
    bank.sysex[getPatchStart(patchNum)+23]=(byte)(p.sysex[6+46]*16+p.sysex[6+45]);           // Breath Control Assign ..(0- 7)| Breath Control Sensitivity ..(0-15)
    ...
    bank.sysex[getPatchStart(patchNum)+31]=(byte)(((p.sysex[6+34]&15)>>3)*64);		     // Pitch Bend Step (High)..(0-12)
                                                                                             ...
                                                                                             //    ***** Common *****
    bank.sysex[getPatchStart(patchNum)+32]=(byte)(p.sysex[6+61]*4+(p.sysex[6+60]&3));        // Voice Memory Select ....(0- 1)| Key Assign Mode ...(0- 2)
