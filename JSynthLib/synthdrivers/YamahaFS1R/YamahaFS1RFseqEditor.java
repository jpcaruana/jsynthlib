...
class YamahaFS1RFseqEditor extends PatchEditorFrame
{
	...
	private Container buildHeaderWindow() {
		...
		oPanel2.add(new KnobWidget("Data end", p, 0, 512, 0, new YamahaFS1RFseqDriver.DoubleModel(p, 0x1E), new YamahaFS1RFseqDriver.Sender(0x1E)));
		oPanel2.add(new KnobWidget("Loop start", p, 0, 512, 0, new YamahaFS1RFseqDriver.DoubleModel(p, 0x10), new YamahaFS1RFseqDriver.Sender(0x10)));
		oPanel2.add(new KnobWidget("Loop end", p, 0, 512, 0, new YamahaFS1RFseqDriver.DoubleModel(p, 0x12), new YamahaFS1RFseqDriver.Sender(0x12)));
		oPanel2.add(new KnobWidget("Speed adjust", p, 0, 127, -64, new YamahaFS1RFseqDriver.Model(p, 0x15), new YamahaFS1RFseqDriver.Sender(0x15)));
		oPanel2.add(new KnobWidget("Tempo vel sens", p, 0, 127, 0, new YamahaFS1RFseqDriver.Model(p, 0x16), new YamahaFS1RFseqDriver.Sender(0x16)));
		oPanel.add(oPanel2);
		
		JPanel oPanel3 = new JPanel(new FlowLayout(FlowLayout.LEFT));
		oPanel3.add(new ComboBoxWidget("Formant note", p, new YamahaFS1RFseqDriver.Model(p, 0x18), new YamahaFS1RFseqDriver.Sender(0x18), YamahaFS1RPerformanceEditor.mNotes));
		oPanel3.add(new SpinnerWidget("Pitch tuning", p, 0, 126, -63, new YamahaFS1RFseqDriver.Model(p, 0x19), new YamahaFS1RFseqDriver.Sender(0x19)));
		oPanel3.add(new KnobWidget("Delay", p, 0, 0x63, 0, new YamahaFS1RFseqDriver.Model(p, 0x1A), new YamahaFS1RFseqDriver.Sender(0x1A)));
		oPanel.add(oPanel3);
	...
	class GraphDisplay extends Canvas 
	{
		...
		void getSequence(String aSeq) {
			...
			if (aSeq.equals(SEQ_PITCH)) {
				mNbOp = 1;
				mY[0] = YamahaFS1RFseqDriver.getPitch(p);
				if (mX == null) {
				...
				for (int op = 0; op < mNbOp; op++) {
					if (aSeq.equals(SEQ_VOICEDFREQ)) {
						mY[op] = YamahaFS1RFseqDriver.getFrequencies(p, op, true);
					}
					else if (aSeq.equals(SEQ_VOICEDLEVEL)) {
						mY[op] = YamahaFS1RFseqDriver.getLevels(p, op, true);
					}
					else if (aSeq.equals(SEQ_UNVOICEDFREQ)) {
						mY[op] = YamahaFS1RFseqDriver.getFrequencies(p, op, false);
					}
					else {
						mY[op] = YamahaFS1RFseqDriver.getLevels(p, op, false);
					}
					if (mX == null) {
