...
public class EnvelopeWidget extends SysexWidget {
    ...
    public void setSliderNum(int num) {
        ...
        setToolTipText("Bank " + ((num - 1) / 16)
		       + "  Sliders " + (((num - 1) % 16) + 1)
		       + " to " + (((num - 1) % 16) + numFaders));
        for (int j = 0; j < numValues; j++)
	    values[j].setToolTipText("Bank " + ((num - 1) / 16)
				     + "  Slider " + (((num - 1 + j) % 16) + 1));
        for (int j = 0; j < numValues; j++)
	    valueLabels[j].setToolTipText("Bank " + ((num - 1) / 16)
					  + "  Slider " + (((num - 1 + j) % 16) + 1));
    }
    ...
    public void setValue(int fader, int value) {
		...
		if (fader == j) {
		    value = (int) (nodes[i].minX
				   + ((float) (value) / 127
				      * (nodes[i].maxX - nodes[i].minX)));
		    nodes[i].ofsX.set(value);
		    values[j].setText(new Integer(value).toString());
		    try {
			nodes[i].senderX.channel
			    = (byte) ((Device) PatchEdit.appConfig.getDevice(patch.deviceNum)).getChannel();
			PatchEdit.MidiOut.writeLongMessage
			    (((Device) PatchEdit.appConfig.getDevice(patch.deviceNum)).getPort(),
			     nodes[i].senderX.generate(nodes[i].ofsX.get()));
		    } catch (Exception e1) {
		...
		if (fader == j) {
		    ...
		    values[j].setText(new Integer(value).toString());
		    try {
			nodes[i].senderY.channel
			    = (byte) ((Device) PatchEdit.appConfig.getDevice(patch.deviceNum)).getChannel();
			PatchEdit.MidiOut.writeLongMessage
			    (((Device) PatchEdit.appConfig.getDevice(patch.deviceNum)).getPort(),
			     nodes[i].senderY.generate(nodes[i].ofsY.get()));
		    } catch (Exception e1) {
			ErrorMsg.reportStatus(e1);
                    }
                }
    ...
    class EnvelopeCanvas extends JPanel {
	    ...
	    float scaleY;
            for (int i = 0; i < nodes.length; i++) {
		maxX += nodes[i].maxX;
		if ((nodes[i].maxY + nodes[i].baseY > maxY)
		    && (nodes[i].maxY != 5000))
		...
		if ((i > 0) && (g != null))
		    g.drawLine((int) ((currX + x) * scaleX),
			       currentHeight - ((int) (y * scaleY)),
			       (int) ((lastX) * scaleX),
			       currentHeight - ((int) (lastY * scaleY)));
		lastX = x + currX;
		lastY = y;
		currX += x;
            }
	    ...
	    int oldy;
            ...
            public void mousePressed(MouseEvent e) {
                ...
                int y = e.getY();
                for (int i = 0; i < nodes.length; i++)
                    if (((Math.abs(x - nodeX[i])) < 5)
			&& ((Math.abs(y - nodeY[i]) < 5))) {
			dragNode = i;
		    }
                repaint();
            }
		    ...
		    return;
                if (x - oldx > 0) {
		    while ((x - nodeX[dragNode] > 5)
			   && (getX(dragNode) < nodes[dragNode].maxX)) {
			if (nodes[dragNode].invertX)
		    ...
		    while ((x - nodeX[dragNode] < -5)
			   && (getX(dragNode) > nodes[dragNode].minX)) {
			if (nodes[dragNode].invertX)
			    nodes[dragNode].ofsX.set(nodes[dragNode].ofsX.get() + 1);
			else
			    nodes[dragNode].ofsX.set(nodes[dragNode].ofsX.get() - 1);
		    ...
		    while ((y - nodeY[dragNode] < -5)
			   && (getY(dragNode) < nodes[dragNode].maxY + nodes[dragNode].baseY)) {
		...
		updateSize(e);
            ...
            void updateSize(MouseEvent e) {
                if (nodes[dragNode].senderX != null) {
                    try {
			nodes[dragNode].senderX.channel
			    = (byte) ((Device) PatchEdit.appConfig.getDevice(patch.deviceNum)).getChannel();
			PatchEdit.MidiOut.writeLongMessage
			    (((Device) PatchEdit.appConfig.getDevice(patch.deviceNum)).getPort(),
		...
		if (nodes[dragNode].senderY != null) {
			...
			PatchEdit.MidiOut.writeLongMessage
			    (((Device) PatchEdit.appConfig.getDevice(patch.deviceNum)).getPort(),
			     nodes[dragNode].senderY.generate(nodes[dragNode].ofsY.get()));
		    } catch (Exception e2) {
