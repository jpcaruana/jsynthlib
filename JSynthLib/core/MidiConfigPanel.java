...
/** The panel that configures the MIDI layer. Taken out of
 *  PrefsDialog.
 * @author Joe Emenaker
 * @version $Id$
 */
public class MidiConfigPanel extends /* TODO org.jsynthlib.*/ConfigPanel {
	...
	/**
	 * This allows other classes to sign up to be notified any time the midi Driver
	 * changes. - emenake 3/12/2003
	 * @param listener
	 */
	...
	*/
	...
	/**
	 * This handles the issues surrounding switching to a new driver. This breaks down
	 * to several tasks:
	 * 1 - Make sure that we weren't asked to switch to the driver we're already using
	...
	private void midiDriverSelected(MidiWrapper selectedDriver) {
		...
		if(currentDriver != null) {
			...
			if(currentDriver.toString().equals(selectedDriver.toString())) {
				// There's already a driver in use and it's the one they selected. Do nothing.
				System.out.println("We're already using that driver");
				return;
			}
		}
		// Either the current driver is null, or they selected a different driver from the current one
		if(currentDriver != null) {
			currentDriver.close();
		}
	...
	/**
	 * This initializes a new midi driver. Basically, it just trys to call the "init(int,int)"
	 * method on it and, provided that there weren't any exceptions, switch the currentDriver
	 * variable to hold the new driver object
	 * - emenaker 2003.03.19
	 * 
	...
	private void initNewMidiDriver(MidiWrapper newDriver) {
        ...
        try {
			...
			// If an exception wasn't thrown by now, then I guess the driver change went smoothly.
	...
	/**
	 * This returns the currently-selected midi wrapper. At present, it's only used by the main app
	 * to obtain the midi driver after the initial instantiation (ie, before it ever gets a
	...
	/**
	 * This checks if the first item in the combobox is a DoNothingMidiWrapper. If so, it switches
	 * to it. - emenaker 2003.03.19
	 *
	 */
