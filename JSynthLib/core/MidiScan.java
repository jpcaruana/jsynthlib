...
public class MidiScan extends  Thread {
    ...
    private static final int MAXCHANNEL = 16;	// should be 0x7D, but...
    private static final int WAITFORRESPONSE = 100; // Wait at least 100ms for a response from a device

    private SynthTableModel model;
    private javax.swing.ProgressMonitor pb;
	...
	//         if (pb!=null) pb.setMaximum(maxin+maxout-1);// Preparation for a progress bar
        if (pb != null)
	    pb.setMaximum(maxout - 1); // Preparation for a progress bar
	/*
	  try {
