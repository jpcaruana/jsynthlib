...
java.io.Serializable {
	...
	public int getDefaultPadding() { return default_padding; }
	public void setDefaultPadding( int _padding ) { default_padding = _padding; }
	...
	protected boolean addComponent(String id, Point p) {
		...
		if (id.startsWith("Strut")) {
			component = new Strut(p.x, p.y);
			((SpringLayout)c.getLayout())
			.addLayoutComponent(component,
					((Strut)component).getConstraints());
			...
			addWidget(s);
			component.add(s);
			((SpringLayout)component.getLayout())
			.addLayoutComponent(s,
					s.getConstraints());
	...
	protected void setConstraints(JComponent component, Point p, Container c) {
		...
		if (parent == c) {
			new Anchor(component, Anchor.NORTH,
					c, Anchor.NORTH, default_padding*2);
			new Anchor(component, Anchor.WEST,
					c, Anchor.WEST, default_padding*2);
		} else {
	...
	public void createNSAnchor(Component c, Component target, int side) {
		...
		if (ta != null && ta.getConstrainedSide() == side) {
			...
			new Anchor(c, Anchor.opposite(side),
					target, side, default_padding);
	...
	protected Container getContainer(Point p) {
		...
		while (new_container != null
			...
			new_point = SwingUtilities.convertPoint(c, new_point, new_container);
	...
	public void paint(Graphics g) {
		try {
			...
			if (selected != null) {
				...
				Point origin = SwingUtilities.convertPoint(selected.getParent(),
						new Point(0,0),
						this);
	...
	throws Throwable{
		...
		if (args.length == 1 && args[0] instanceof MouseEvent) {
			...
			if (!sp.getViewport()
				...
				Component c =
					SwingUtilities.getDeepestComponentAt(designer.getLayeredPane(),
							p.x,
							p.y);
				if (c == null)
					return null;
				c.dispatchEvent(
						SwingUtilities.convertMouseEvent(this,
								e,
								c));
	...
	static Set getCommonWidgets() { return widgets; }
	
	public static Widget[] getWidgetArray() {
		PanelWidget ifw =
			EditorBuilder.getDesignerFrame().getRootWidget();
		widgets.remove(ifw);
	...
	public static String getPatchIdentifier() { return "p"; }
...
class WidgetComboBoxModel implements ComboBoxModel {
		...
		try {
			...
			while (it.hasNext()) {
				try {
					...
					l.contentsChanged(
							new ListDataEvent(EditorBuilder.getDesignerFrame(),
	...
	public Object getElementAt(int index) { return ids[index]; }
	public Object getSelectedItem() { if (selection > ids.length) return null; return ids[selection]; }
	...
	public static boolean isNotifying() {return notifying;}
}
